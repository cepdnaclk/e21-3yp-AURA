...
const login = useCallback(async (username, password) => {
  ...
  try {
    ...
    // Auto-create a walk-in session for every table login so orders are
    // always scoped to one customer, not to the table for the whole day.
    if (role === 'table' && tableNumber) {
      const tableId = parseInt(tableNumber.replace(/\D/g, ''), 10);
      try {
        const { sessionId } = await walkInAPI.createSession(tableId);
        session.walkInSessionId = sessionId;
      } catch (e) {
        console.warn('[AURA] Could not create walk-in session:', e.message);
      }
    }
